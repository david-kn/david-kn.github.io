...
B) If such an option is not possible due to any other reasoning it is possible to disable such a behaviour _ad hoc_ and explicitely for individual pipelines by `changelog=false` parameter  within pipeline include statement, e.g.: 
